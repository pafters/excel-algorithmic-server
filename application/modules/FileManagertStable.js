...
class FileManager {
    ...
    removeIdentical = (products) => {
        ...
        for (const product of products) {
            ...
            const canonicalProduct = product?.product_name.toLowerCase();

            // Проверим находится ли уже строка в массиве уникальных записей.
            if (!uniqueProducts.hasOwnProperty(canonicalProduct)) {
                // Если нет, добавим продукт в объект и в новый массив uniqueProductsArray.
                uniqueProducts[canonicalProduct] = true;
    ...
    removeDuplicates = async (products) => {
        let uniqueProducts = this.removeIdentical(products);
        console.log('1этап', products.length, uniqueProducts.length)
    ...
    fileUpload = async (data) => {
        ...
        let mainArray = await this.removeDuplicates(data);
